PROGRAM StokesALE
  ...
  INTEGER(OC_Intg), PARAMETER :: DependentFieldUserNumberStokes=8
  INTEGER(OC_Intg), PARAMETER :: DependentFieldUserNumberMovingMesh=9
  INTEGER(OC_Intg), PARAMETER :: MaterialsFieldUserNumberStokes=10
  INTEGER(OC_Intg), PARAMETER :: MaterialsFieldUserNumberMovingMesh=11
  INTEGER(OC_Intg), PARAMETER :: IndependentFieldUserNumberStokes=12
  INTEGER(OC_Intg), PARAMETER :: IndependentFieldUserNumberMovingMesh=13
  INTEGER(OC_Intg), PARAMETER :: EquationsSetUserNumberStokes=14
  INTEGER(OC_Intg), PARAMETER :: EquationsSetUserNumberMovingMesh=15
  INTEGER(OC_Intg), PARAMETER :: EquationsSetFieldUserNumberStokes=16
  INTEGER(OC_Intg), PARAMETER :: EquationsSetFieldUserNumberMovingMesh=17
  INTEGER(OC_Intg), PARAMETER :: ProblemUserNumber=18
  INTEGER(OC_Intg), PARAMETER :: GeneratedMeshUserNumber=19
  INTEGER(OC_Intg), PARAMETER :: AnalyticFieldUserNumber=20

  INTEGER(OC_Intg), PARAMETER :: DomainUserNumber=1
  INTEGER(OC_Intg), PARAMETER :: SolverMovingMeshUserNumber=1
  INTEGER(OC_Intg), PARAMETER :: SolverStokesUserNumber=2
  ...
  INTEGER(OC_Intg), PARAMETER :: MaterialsFieldUserNumberMovingMeshK=1
  ...
  INTEGER(OC_Intg) :: NUMBER_OF_MOVED_WALL_NODES_STOKES
  INTEGER(OC_Intg) :: NUMBER_OF_INLET_WALL_NODES_STOKES
  INTEGER(OC_Intg) :: NUMBER_OF_FIXED_WALL_NODES_MOVING_MESH
  INTEGER(OC_Intg) :: NUMBER_OF_MOVED_WALL_NODES_MOVING_MESH

  INTEGER(OC_Intg) :: EQUATIONS_STOKES_OUTPUT
  INTEGER(OC_Intg) :: EQUATIONS_MOVING_MESH_OUTPUT
  ...
  INTEGER(OC_Intg) :: DYNAMIC_SOLVER_STOKES_INPUT_OPTION
  ...
  INTEGER(OC_Intg) :: LINEAR_SOLVER_MOVING_MESH_OUTPUT_TYPE
  ...
  TYPE(OC_FieldType) :: DependentFieldMovingMesh
  TYPE(OC_FieldType) :: MaterialsFieldStokes
  TYPE(OC_FieldType) :: MaterialsFieldMovingMesh
  TYPE(OC_FieldType) :: IndependentFieldStokes
  TYPE(OC_FieldType) :: IndependentFieldMovingMesh
  TYPE(OC_FieldType) :: EquationsSetFieldStokes
  TYPE(OC_FieldType) :: EquationsSetFieldMovingMesh
  TYPE(OC_BoundaryConditionsType) :: BoundaryConditionsStokes
  TYPE(OC_BoundaryConditionsType) :: BoundaryConditionsMovingMesh
  TYPE(OC_EquationsSetType) :: EquationsSetStokes
  TYPE(OC_EquationsSetType) :: EquationsSetMovingMesh
  TYPE(OC_EquationsType) :: EquationsStokes
  TYPE(OC_EquationsType) :: EquationsMovingMesh
  ...
  TYPE(OC_SolverType) :: LinearSolverMovingMesh
  TYPE(OC_SolverEquationsType) :: SolverEquationsStokes
  TYPE(OC_SolverEquationsType) :: SolverEquationsMovingMesh
  ...
  INITIAL_FIELD_MOVING_MESH(1)=0.0_OC_RP
  INITIAL_FIELD_MOVING_MESH(2)=0.0_OC_RP
  INITIAL_FIELD_MOVING_MESH(3)=0.0_OC_RP
  ...
  IF(MOVED_WALL_NODES_MOVING_MESH_FLAG) THEN
     ...
     BOUNDARY_CONDITIONS_MOVING_MESH(1)=0.0_OC_RP
     BOUNDARY_CONDITIONS_MOVING_MESH(2)=0.0_OC_RP
     BOUNDARY_CONDITIONS_MOVING_MESH(3)=0.0_OC_RP
  ENDIF
  !Set material parameters
  MU_PARAM_STOKES=1.0_OC_RP
  RHO_PARAM_STOKES=1.0_OC_RP
  K_PARAM_MOVING_MESH=1.0_OC_RP
  ...
  LINEAR_SOLVER_MOVING_MESH_OUTPUT_TYPE=OC_SOLVER_NO_OUTPUT
  ...
  EQUATIONS_MOVING_MESH_OUTPUT=OC_EQUATIONS_NO_OUTPUT
  !Set time parameter
  ...
  CALL OC_Field_Initialise(EquationsSetFieldStokes,Err)
  CALL OC_EquationsSet_Initialise(EquationsSetStokes,Err)
  CALL OC_EquationsSet_CreateStart(EquationsSetUserNumberStokes,Region,GeometricField, &
       & [OC_EQUATIONS_SET_FLUID_MECHANICS_CLASS,OC_EQUATIONS_SET_STOKES_EQUATION_TYPE,OC_EQUATIONS_SET_ALE_STOKES_SUBTYPE], &
  ...
  CALL OC_Field_Initialise(EquationsSetFieldMovingMesh,Err)
  CALL OC_EquationsSet_Initialise(EquationsSetMovingMesh,Err)
  CALL OC_EquationsSet_CreateStart(EquationsSetUserNumberMovingMesh,Region,GeometricField, &
       & [OC_EQUATIONS_SET_CLASSICAL_FIELD_CLASS,OC_EQUATIONS_SET_LAPLACE_EQUATION_TYPE, &
       & OC_EQUATIONS_SET_MOVING_MESH_LAPLACE_SUBTYPE],EquationsSetFieldUserNumberMovingMesh,EquationsSetFieldMovingMesh, &
       & EquationsSetMovingMesh,Err)
  !Set the equations set to be a moving mesh problem
  !Finish creating the equations set
  CALL OC_EquationsSet_CreateFinish(EquationsSetMovingMesh,Err)
  ...
  CALL OC_Field_Initialise(DependentFieldMovingMesh,Err)
  CALL OC_EquationsSet_DependentCreateStart(EquationsSetMovingMesh,DependentFieldUserNumberMovingMesh, &
       & DependentFieldMovingMesh,Err)
  !Set the mesh component to be used by the field components.
  DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
     CALL OC_Field_ComponentMeshComponentSet(DependentFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,COMPONENT_NUMBER, &
          & MESH_COMPONENT_NUMBER_SPACE,Err)
     CALL OC_Field_ComponentMeshComponentSet(DependentFieldMovingMesh,OC_FIELD_DELUDELN_VARIABLE_TYPE,COMPONENT_NUMBER, &
          & MESH_COMPONENT_NUMBER_SPACE,Err)
  ENDDO
  !Finish the equations set dependent field variables
  CALL OC_EquationsSet_DependentCreateFinish(EquationsSetMovingMesh,Err)

  !Initialise dependent field
  DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
     CALL OC_Field_ComponentValuesInitialise(DependentFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,OC_FIELD_VALUES_SET_TYPE, &
  ...
  CALL OC_Field_VariableLabelSet(MaterialsFieldStokes,OC_FIELD_U_VARIABLE_TYPE,"MaterialsALE",Err)
  ...
  CALL OC_Field_Initialise(MaterialsFieldMovingMesh,Err)
  CALL OC_EquationsSet_MaterialsCreateStart(EquationsSetMovingMesh,MaterialsFieldUserNumberMovingMesh, &
       & MaterialsFieldMovingMesh,Err)
  CALL OC_Field_VariableLabelSet(MaterialsFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,"MaterialsMovingMesh",Err)
  !Finish the equations set materials field variables
  CALL OC_EquationsSet_MaterialsCreateFinish(EquationsSetMovingMesh,Err)
  CALL OC_Field_ComponentValuesInitialise(MaterialsFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,OC_FIELD_VALUES_SET_TYPE, &
  ...
  CALL OC_Field_Initialise(IndependentFieldStokes,Err)
  CALL OC_EquationsSet_IndependentCreateStart(EquationsSetStokes,IndependentFieldUserNumberStokes,IndependentFieldStokes,Err)
  CALL OC_Field_VariableLabelSet(IndependentFieldStokes,OC_FIELD_U_VARIABLE_TYPE,"ALE",Err)
  !Set the mesh component to be used by the field components.
  DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
     CALL OC_Field_ComponentMeshComponentSet(InDependentFieldStokes,OC_FIELD_U_VARIABLE_TYPE,COMPONENT_NUMBER, &
          & MESH_COMPONENT_NUMBER_SPACE,Err)
  ENDDO
  !Finish the equations set independent field variables
  CALL OC_EquationsSet_IndependentCreateFinish(EquationsSetStokes,Err)

  !Create the equations set independent field variables for moving mesh
  CALL OC_Field_Initialise(IndependentFieldMovingMesh,Err)
  CALL OC_EquationsSet_IndependentCreateStart(EquationsSetMovingMesh,IndependentFieldUserNumberMovingMesh, &
       & IndependentFieldMovingMesh,Err)
  CALL OC_Field_VariableLabelSet(IndependentFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,"MovingMesh",Err)
  !Set the mesh component to be used by the field components.
  DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
     CALL OC_Field_ComponentMeshComponentSet(IndependentFieldMovingMesh,OC_FIELD_U_VARIABLE_TYPE,COMPONENT_NUMBER, &
          & MESH_COMPONENT_NUMBER_SPACE,Err)
  ENDDO
  !Finish the equations set independent field variables
  CALL OC_EquationsSet_IndependentCreateFinish(EquationsSetMovingMesh,Err)
  ...
  !Create the equations set equations
  CALL OC_Equations_Initialise(EquationsMovingMesh,Err)
  CALL OC_EquationsSet_EquationsCreateStart(EquationsSetMovingMesh,EquationsMovingMesh,Err)
  !Set the equations matrices sparsity type
  CALL OC_Equations_SparsityTypeSet(EquationsMovingMesh,OC_EQUATIONS_SPARSE_MATRICES,Err)
  !Set the equations set output
  CALL OC_Equations_OutputTypeSet(EquationsMovingMesh,EQUATIONS_MOVING_MESH_OUTPUT,Err)
  !Finish the equations set equations
  CALL OC_EquationsSet_EquationsCreateFinish(EquationsSetMovingMesh,Err)
  ...
  CALL OC_Problem_CreateStart(ProblemUserNumber,context,[OC_PROBLEM_FLUID_MECHANICS_CLASS,OC_PROBLEM_STOKES_EQUATION_TYPE, &
       & OC_PROBLEM_ALE_STOKES_SUBTYPE],Problem,Err)
  ...
  CALL OC_ControlLoop_TimeInputSet(ControlLoop,DYNAMIC_SOLVER_STOKES_INPUT_OPTION,Err)
  ...
  CALL OC_Solver_Initialise(LinearSolverMovingMesh,Err)
  ...
  CALL OC_Problem_SolverGet(Problem,OC_CONTROL_LOOP_NODE,SolverMovingMeshUserNumber,LinearSolverMovingMesh,Err)
  !Set the output type
  !CALL OC_Solver_OutputTypeSet(LinearSolverMovingMesh,LINEAR_SOLVER_MOVING_MESH_OUTPUT_TYPE,Err)
  CALL OC_Solver_OutputTypeSet(LinearSolverMovingMesh,4,Err)
  !Set the solver settings
  IF(LINEAR_SOLVER_MOVING_MESH_DIRECT_FLAG) THEN
     CALL OC_Solver_LinearTypeSet(LinearSolverMovingMesh,OC_SOLVER_LINEAR_DIRECT_SOLVE_TYPE,Err)
     CALL OC_Solver_LibraryTypeSet(LinearSolverMovingMesh,OC_SOLVER_MUMPS_LIBRARY,Err)
  ELSE
     CALL OC_Solver_LinearTypeSet(LinearSolverMovingMesh,OC_SOLVER_LINEAR_ITERATIVE_SOLVE_TYPE,Err)
     CALL OC_Solver_LinearIterativeMaximumIterationsSet(LinearSolverMovingMesh,MAXIMUM_ITERATIONS,Err)
     CALL OC_Solver_LinearIterativeDivergenceToleranceSet(LinearSolverMovingMesh,DIVERGENCE_TOLERANCE,Err)
     CALL OC_Solver_LinearIterativeRelativeToleranceSet(LinearSolverMovingMesh,RELATIVE_TOLERANCE,Err)
     CALL OC_Solver_LinearIterativeAbsoluteToleranceSet(LinearSolverMovingMesh,ABSOLUTE_TOLERANCE,Err)
     CALL OC_Solver_LinearIterativeGMRESRestartSet(LinearSolverMovingMesh,RESTART_VALUE,Err)
  ENDIF
  ...
  CALL OC_Solver_Initialise(LinearSolverMovingMesh,Err)
  CALL OC_Solver_Initialise(DynamicSolverStokes,Err)
  CALL OC_SolverEquations_Initialise(SolverEquationsMovingMesh,Err)
  ...
  CALL OC_Problem_SolverGet(Problem,OC_CONTROL_LOOP_NODE,SolverMovingMeshUserNumber,LinearSolverMovingMesh,Err)
  CALL OC_Solver_SolverEquationsGet(LinearSolverMovingMesh,SolverEquationsMovingMesh,Err)
  !Set the solver equations sparsity
  CALL OC_SolverEquations_SparsityTypeSet(SolverEquationsMovingMesh,OC_SOLVER_SPARSE_MATRICES,Err)
  !Add in the equations set
  CALL OC_SolverEquations_EquationsSetAdd(SolverEquationsMovingMesh,EquationsSetMovingMesh,EquationsSetIndex,Err)
  ...
  IF(MOVED_WALL_NODES_STOKES_FLAG) THEN
     DO NODE_COUNTER=1,NUMBER_OF_MOVED_WALL_NODES_STOKES
        ...
        CONDITION=OC_BOUNDARY_CONDITION_MOVED_WALL
        CALL OC_Decomposition_NodeDomainGet(Decomposition,1,NODE_NUMBER,BoundaryNodeDomain,Err)
        IF(BoundaryNodeDomain==ComputationalNodeNumber) THEN
           DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
              VALUE=0.0_OC_RP
  ...
  CALL OC_BoundaryConditions_Initialise(BoundaryConditionsMovingMesh,Err)
  CALL OC_SolverEquations_BoundaryConditionsCreateStart(SolverEquationsMovingMesh,BoundaryConditionsMovingMesh,Err)
  !Set fixed wall nodes
  IF(FIXED_WALL_NODES_MOVING_MESH_FLAG) THEN
     DO NODE_COUNTER=1,NUMBER_OF_FIXED_WALL_NODES_MOVING_MESH
        ...
        IF(BoundaryNodeDomain==ComputationalNodeNumber) THEN
           DO COMPONENT_NUMBER=1,NUMBER_OF_DIMENSIONS
              VALUE=0.0_OC_RP
              CALL OC_BoundaryConditions_SetNode(BoundaryConditionsMovingMesh,DependentFieldMovingMesh, &
                   & OC_FIELD_U_VARIABLE_TYPE,1,OC_NO_GLOBAL_DERIV,NODE_NUMBER,COMPONENT_NUMBER,CONDITION,VALUE,Err)
           ENDDO
        ENDIF
     ENDDO
  ENDIF
  ...
  !Finish the creation of the equations set boundary conditions
  CALL OC_SolverEquations_BoundaryConditionsCreateFinish(SolverEquationsMovingMesh,Err)
  ...
  IF(EXPORT_FIELD_IO) THEN
     ...
     CALL OC_Fields_NodesExport(Fields,"stokes_ale","FORTRAN",Err)
     CALL OC_Fields_ElementsExport(Fields,"stokes_ale","FORTRAN",Err)
